...
@if (isset($allAvailableRoom) && !empty($allAvailableRoom))
    @foreach ($allAvailableRoom as $index => $room)
        ...
        @if ($room['rooms']['roomImages']->count() > 0 && checkImageExists($room['rooms']['roomImages']))
            ...
            <span class="roomCount" data-room_count="{{ $room['room_count'] }}"></span>

            <div class="main-box mb-xl-3 mb-3 bor">
                <div class="row g-0">
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-4">
                        <strong class="badge-left">{{ $room['availableRoom'] }}
                            {{ $room['availableRoom'] > 1 ? 'Rooms' : 'Room' }} Left</strong>
                        <div class="room-img" id="room-img">
                            @if (count($room['rooms']['roomImages']) > 0)
                                @foreach ($room['rooms']['roomImages'] as $roomImg)
                                    <a href="javascript:void(0);" data-bs-toggle="modal"
                                        data-bs-target="#room-modal-{{ $index }}">

                                        <div class="image bg-image-style lazy"
                                            data-bg="{{ asset('storage/' . $roomImg->image) }}"></div>
                                    </a>
                                @endforeach
                            @else
                                <a href="{{ asset('assets/media/no-hotel-img.svg') }}" data-bs-toggle="modal"
                                    data-bs-target="#room-modal" data-title="Room Image">
                        ...
                        <div class="center-section">
                            ...
                            <div class="title mt-xl-3 mt-3">
                                ...
                                <span class="pt-2 d-block">
                                    ...
                                    <strong>Ideal for {{ $room['rooms']?->stay_guest }}
                                        {{ $room['rooms']?->stay_guest > 0 ? 'Guests' : 'Guest' }}</strong> <br />
                                </span>
                                ...
                                @if (!empty($room['rooms']['addAmenity']) && count($room['rooms']['addAmenity']) > 0)
                                    ...
                                    <ul class="mt-3 main-ul">
                                        ...
                                        @if (count($room['rooms']['addAmenity']) > 4)
                                            <li><a data-bs-toggle="modal" data-bs-target="#room-amenities_{{ $index }}" href="javascript:void(0);"
                                             title="More amenities">{{ count($room['rooms']['addAmenity']) - 4 }}more</a></li>
                                        @endif
                                    </ul>
                                @endif
                            </div>
                    ...
                    <div class="modal fade hotel-amenities" id="room-amenities_{{ $index }}" tabindex="-1"
                        aria-labelledby="exampleModalLabel">
                        <div class="modal-dialog modal-lg modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header w-100">
                                    <div class=" w-100 position-relative">
                                        <h4 class="modal-title ">{{ ucwords($room['rooms']?->hotel?->name) ?? '' }}
                                        </h4>
                                            ...
                                            <div class="repeat-box">
                                                <div class="d-flex justify-content-between w-100 align-items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div id="select-{{ $title['id'] }}-{{ $index + 1 }}">
                                                            <a href="javascript:void(0);"
                                                                data-section="{{ $index + 1 }}"
                                                                data-id="{{ $title['id'] }}"
                                                                data-group="{{ $title['category'] }}"
                                                                class="btn btn-black select-room-btn {{ $key }}"
                                                                title="Select"
                                                                data-availability="{{ $room['availableRoom'] }}">
                                                                Select
                                                            </a>
                                                        </div>
                                                            ...
                                                            <div class="quantity-section"
                                                                ...
                                                                <div class="d-flex justify-content-between w-100">
                                                                    <input type="hidden"
                                                                        ...
                                                                        value="{{ $room['room_count'] }}" readonly>
                                                                    <button class="btn btn-black unselect-room-btn"
                                                                        data-section="{{ $index + 1 }}"
                                                                        data-id="{{ $title['id'] }}"
                                                                        data-availability="{{ $room['availableRoom'] }}">
                                                                        Unselect  
            ...
            @if (!empty($room))
                <div class="modal fade fullscreen-modal" id="room-modal-{{ $index }}" data-bs-backdrop="static"
                    tabindex="-1" data-bs-focus="false">
                    <div class="modal-dialog modal-fullscreen modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header bg-primary text-white">
                                <h5 class="modal-title text-black" id="galleryModalLabel">
                                    {{ ucwords($room['rooms']?->hotel?->name) ?? '' }}</h5>
                                    ...
                                    @if (count($room['rooms']['roomImages']) > 0)
                                        @foreach ($room['rooms']['roomImages'] as $roomImg)
                                            <div class="col-12 col-sm-6 col-md-3 col-xl-3">
                                                <a href="javascript:void(0);"
                                                    data-bs-target="#room-modal-data-{{ $index }}"
                                        ...
                                        <div class="col-12 col-sm-6 col-md-3 col-xl-3">
                                            @if (isset($hotelImg['image']))
                                                <a href="{{ asset('storage/' . $hotelImg['image']) }}"
                                                    data-bs-target="#room-modal-data" data-bs-toggle="modal"
            ...
            <div class="modal fade fullscreen-modal slider-modal" id="room-modal-data-{{ $index }}"
                                ...
                                <div class="row d-flex justify-content-center">
                                    <div class="col-12">
                                        <div class="slider-nav-{{ $index }}" style="display: none;">
                                            @if (count($room['rooms']['roomImages']) > 0)
                                                @foreach ($room['rooms']['roomImages'] as $roomImg)
                                                    <div class="image"> <img src="{{ asset('storage/' . $roomImg->image) }}" alt="{{ ImageAltTag($room['rooms']?->hotel, $roomImg->alt_tag) }}">
                                                    </div>
                                                @endforeach
                                            @else
                                                <div class="image">
