...
<head>
    ...
    <title>BadrinathHotels - Your Booking is Cancellation is Under Process!</title>
...
<body style="font-family: Helvetica, Arial, sans-serif; margin: 20px 0px; padding: 10px 10px; border: 0; background-color:#F3F2F1;">
    <div class="email-container" style="font-family: Helvetica, Arial, sans-serif; width: 100%; max-width:600px; margin: 0px auto; padding: 0px 0px; border: 1px solid #EEEEEE; background:#fff;">
        <div class="box" style="font-family: Helvetica, Arial, sans-serif;  border: 1px solid #EEEEEE; margin:5px; padding:20px 20px; border-radius:5px; box-shadow: rgba(99, 99, 99, 0.1) 0px 2px 4px 0px; border: 1px solid #EEEEEE; background:#fff!important;">
            ...
            <div class="header">
                <img style="font-family: Helvetica, Arial, sans-serif; max-width: 200px;" src="{{ !empty($message) ? $message->embed(asset('assets/front/images/black-logo.png')) : asset('assets/front/images/black-logo.png') }}" alt="BadrinathHotels Logo">
            </div>
            <div class="content">
                ...
                <p style="font-size:24px; color:#05264E; line-height:1.4;"><b style="font-weight:700;">Your cancellation request is under process —</b></p>
                <p style="font-size:12px; line-height: 1.5; color:#3B4C65; font-weight:400; margin-top:10px; font-family: Helvetica, Arial, sans-serif; line-height:1.4;" align="justify">We're sorry to see you go! Your booking at Fairfield by Marriott Goa calamite <b>(Booking ID:  {{ $booking->booking_id }})</b>, for the dates {{ formatDateMdY($booking?->check_in_date) }} {{ $booking->hotel?->check_in_time }} to {{ formatDateMdY($booking?->check_out_date) }} {{ $booking->hotel?->check_out_time }} is being cancelled as per your request.</p>
                <p style="font-size:12px; line-height: 1.5; color:#3B4C65; font-weight:400; margin-top:10px; font-family: Helvetica, Arial, sans-serif; line-height:1.4;" align="justify">We are processing your refund for the amount of  <b>₹ {{ _nf($booking->transactions->sum('amount')) }}</b>. The cancellation has been processed with a penalty of 
                @if( !empty($preview) && $preview == 'true' )
                    <input type="number" name="penaltyAmount" id="penaltyAmount" value="" placeholder="Enter Penalty Amount" />
                @else
                    <b>₹ {{_nf($penaltyAmount)}}</b>
                @endif
                , as per the hotel's cancellation policy.</p>
                <p style="font-size:12px; line-height: 1.5; color:#3B4C65; font-weight:400; margin-top:10px; font-family: Helvetica, Arial, sans-serif; line-height:1.4;" align="justify">Our payment processing partner, RazorPay, will initiate the refund within 24 hours. Please note that it may take 5-7 business days for the refund amount to appear in your bank account, depending upon the bank’s processing capability.</p>

                <p style="color:#05264E;font-family: Helvetica, Arial, sans-serif; font-size:14px; font-weight:500; margin-top:20px;"><b style="padding-right:5px;">Booking Summary</b></p>
                ...
                <div style="background:#F2F2F2; width: 100%; margin: 0 auto; margin-top: 20px; border-radius: 8px;">
                    <table style="background:#F2F2F2 collapse; width: 100%; margin: 0 auto;">
                        <tr>
                            ...
                            <td style="text-align: right; padding: 8px 12px; font-size: 18px; font-weight: 600; color: #1A1F24;">
                                ₹{{ _nf( max(0, ($booking->transactions->sum('amount')-$penaltyAmount)) ) }}
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div style="margin-top:20px; margin-bottom:15px;">                
                    ...
                    <p style="font-family: Helvetica, Arial, sans-serif; color:#78828D; font-size:14px; font-weight:400; padding:0px 0px 0px 0px; margin:0px 0px 5px 0px; line-height:1.5em;">BadrinathHotels Team</p>
                </div>
            ...
            <div style="font-family: Helvetica, Arial, sans-serif; text-align:center; color:#666; font-size: 12px; margin-top: 10px; padding-top:10px; padding-bottom:20px;">
                <div>
                    <a href="https://badrinathhotels.com/terms-and-conditions" style="font-family: Helvetica, Arial, sans-serif;  color:#3B4C65; margin: 0 5px;  display: inline-block;font-size:14px; font-weight:600; text-decoration:underline;">Terms and Conditions</a>&nbsp; &nbsp;<a href="https://badrinathhotels.com/privacy-policy" style="font-family: Helvetica, Arial, sans-serif; color:#3B4C65; font-size:14px; font-weight:600; text-decoration:underline;">Privacy Policy</a>
                    <div class="contact-info" style="font-family: Helvetica, Arial, sans-serif; margin-top: 10px; font-size: 12px;">
                        <p style="font-family: Helvetica, Arial, sans-serif; color:#3B4C65; font-size:14px; text-align-center;">Copyright © {{date('Y')}} | BadrinathHotels | All Rights Reserved</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if( !empty($preview) && $preview == 'true' )
            ...
            <script>
                const isConfirmed = (penaltyAmount) => {
                    let text = "Penalty amount is " + penaltyAmount + "\nPress a button!\nEither OK or Cancel.";
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    document.getElementById('sendButton').addEventListener('click', function() {
                        ...
                        var params = JSON.stringify({ booking_id: '{{$booking->booking_id}}', email_type: '{{$emailType}}', '_token': '{{ csrf_token() }}', penaltyAmount: penaltyAmount });
                        xhr.onload = function() {
